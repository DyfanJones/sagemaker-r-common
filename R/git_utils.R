...
# https://github.com/aws/sagemaker-python-sdk/blob/master/src/sagemaker/git_utils.py
...
.valid_git_config <- function(git_config){
  if (!("repo" %in% names(git_config)))
    ValueError$new("Please provide a repo for git_config.")
  for (key in names(git_config)){
    if (key == "2FA_enabled"){
      if (!inherits(git_config[["2FA_enabled"]], "logical")){
        ValueError$new("Please enter a logical type for 2FA_enabled'.")}
    } else if(!inherits(git_config[[key]], "character")){
      ValueError$new(sprintf("'%s' must be a string.",key))}
  }
}
...
.insert_token_to_repo_url <- function(url, token){
  index = nchar("https://")
  if (gregexpr(token, url)[[1]][1] == index)
    return(url)
...
.checkout_branch_and_commit <- function(git_config, dest_dir){
  ...
  if ("commit" %in% names(git_config)){
    processx::run("git", args = c("checkout", git_config$commit), wd = dest_dir)
  }
}
